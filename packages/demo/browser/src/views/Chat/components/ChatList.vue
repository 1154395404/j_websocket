<template>
  <div class="chatList" v-loading="loading" element-loading-text="正在加载聊天记录...">
    <ul class="chatListScroll" ref="chatListScroll" :class="[$store.state.currentChatTarget.type]">
      <li class="chatItem" :class="{ myself: isMySelf(item.from) }" v-for="(item, index) in chatList" :key="index">
        <el-image class="avatar" :src="getAvatar(item.from)" :preview-src-list="[getAvatar(item.from)]" alt="" />
        <div>

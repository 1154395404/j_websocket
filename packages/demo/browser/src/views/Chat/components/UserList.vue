  ...
  <ul class="userList" ref="userList" v-loading="loadingUserList" element-loading-text="正在加载用户列表...">
    <li class="user " :class="{ active: currentClickId === '-1' }"
      ...
      <div style="
              ...
              border-radius: 6px;
            " class="avatarGroup">
        <lord-icon style="width: 40px; height: 40px" src="https://cdn.lordicon.com/flvisirw.json" trigger="hover">
        </lord-icon>
    ...
    calcGroupWhoSpeak(groupChatInfo.lastMessage.from) + (getLastMessage(groupChatInfo.lastMessage.message) ||
      '群里还没有人发消息哦')
  }}</span>
      </div>
    </li>
    <transition-group v-if="isEnter" enter-active-class="animate__animated animate__backInLeft">
      <li class="user " :style="{ animationDelay: 0.1 * index + 's' }" :class="{ active: user.id === currentClickId }"
        v-for="(user, index) in sortUserList" :key="user.id" @click="changeChatTarget(user, sendType.private)">
    ...
    <template v-else>
      <li class="user " :class="{ active: user.id === currentClickId }" v-for="(user) in sortUserList" :key="user.id"
